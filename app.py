...
# 시간에 따라 분배하는 함수
def distribute_money(hours_worked, total_income, num_dropouts, dropout_hours):
    total_hours_worked = sum(hours_worked)
    total_people = len(hours_worked) - num_dropouts
    ...
    # 중간 이탈자들의 금액 계산
    dropout_income = min_amount_per_person * num_dropouts
    
    # 남은 금액 계산
    remaining_amount = total_income - min_amount_per_person * total_people - dropout_income
    ...
    # 남은 잔돈 계산
    remaining_change = total_income - sum(rounded_incomes) - dropout_income
...
num_dropouts = st.number_input("중간 이탈자 수", min_value=0, step=1, value=0)
...
for i in range(num_people):
    if i < num_dropouts:
        dropout_hours.append(st.number_input(f"{i+1}번째 중간 이탈자의 회식한 시간", min_value=0, step=1))
    else:
...
if st.button("분배하기"):
    # 유효성 검사
    if total_income <= 0 or any(hour < 0 for hour in hours_worked) or any(hour < 0 for hour in dropout_hours):
        st.error("잘못된 입력입니다.")
    else:
        # 각 개인이 받아야 할 돈 계산
        individual_incomes, remaining_change = distribute_money(hours_worked, total_income, num_dropouts, dropout_hours)
