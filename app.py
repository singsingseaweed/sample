...
[ex] 5시59분 시작 6시 00분 퇴장시 1시간으로 간주
[ex] 6시00분 시작 7시 59분 퇴장시 1시간으로 간주

2. 모든 참여자에게 적용되는 최저금액은 총 금액의 n분의1 한 값에 7/11로 적용
3. 지불금액 = (최저금액 + 시간당 추가비용) 으로 계산
4. 시간당 추가비용은 {총 금액-(최저금액 * 참가자수)} 시간당 비율 로 계산
5. 회식 종료 시간은 마지막 주문을 기준
6. 중간 귀가자들은 퇴장시 몇시인지 기록(카톡)
""")
