...
# 시간에 따라 분배하는 함수
def distribute_money(common_hours, less_hours, total_income, total_people):
    # 공통 시간에 근무하는 사람들과 덜 근무하는 사람의 수
    num_people_with_less_hours = len(less_hours)
    total_people = total_people + num_people_with_less_hours
    ...
    for i in range(len(less_hours)):
        individual_incomes[i + num_people_with_less_hours] += remaining_amount * distribution_ratio[i]
...
# 전체 인원 입력 받기
total_people = st.number_input("전체 인원", min_value=1, step=1)
...
if st.button("분배하기"):
    ...
    if total_income <= 0 or common_hours < 0 or any(hour < 0 for hour in less_hours):
        ...
        # 각 개인이 받아야 할 돈 계산
        individual_incomes, remaining_change = distribute_money(common_hours, less_hours, total_income, total_people)

        # 결과 출력
        st.write("공통된 시간을 근무하는 사람: ", f"{total_income / (2 * total_people):,.0f}원")
        for i, income in enumerate(individual_incomes[num_people_with_less_hours:], start=num_people_with_less_hours):
            st.write(f"{i+1}번째 원래보다 덜 근무하는 사람: {income:,.0f}원")
