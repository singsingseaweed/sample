...
# 각 도망간 사람 무리의 그룹별 시간 입력 받기
runaway_hours_groups = []
for i in range(num_runaway_groups):
    runaway_hours_group = st.number_input(f"{i+1}번째 도망간 사람 무리의 그룹별 총 일한 시간을 입력하세요", min_value=0, step=1)
...
if st.button("분배하기"):
    # 유효성 검사
    if total_income <= 0 or hours_worked <= 0 or any(hour < 0 for hour in runaway_hours_groups):
        st.error("잘못된 입력입니다.")
    else:
        # 각 개인이 받아야 할 돈 계산
        total_working_hours = num_people * hours_worked + sum(runaway_hours_groups)
        individual_hours = [hours_worked] * num_people + runaway_hours_groups
